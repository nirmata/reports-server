...
func (c *cpolrdb) Create(ctx context.Context, cpolr *v1alpha2.ClusterPolicyReport) error {
	...
	klog.Infof("DB: Creating entry in primary database for key:%s", cpolr.Name)
	jsonb, err := json.Marshal(*cpolr)
	if err != nil {
		klog.ErrorS(err, "DB: Failed to marshal cpolr")
		return err
	}
	...
	if err != nil {
		klog.ErrorS(err, "DB: Failed to create cpolr in primary database")
		return fmt.Errorf("create clusterpolicyreport: %v", err)
	}
	klog.Infof("DB: Successfully created entry in primary database for key:%s", cpolr.Name)
	return nil
}
...
func (c *cpolrdb) Update(ctx context.Context, cpolr *v1alpha2.ClusterPolicyReport) error {
	...
	klog.Infof("DB: Updating entry in primary database for key:%s", cpolr.Name)
	jsonb, err := json.Marshal(*cpolr)
	if err != nil {
		klog.ErrorS(err, "DB: Failed to marshal cpolr")
		return err
	...
	if err != nil {
		klog.ErrorS(err, "DB: Failed to update cpolr in primary database")
		return fmt.Errorf("update clusterpolicyreport: %v", err)
	}
	klog.Infof("DB: Successfully updated entry in primary database for key:%s", cpolr.Name)
	return nil
}
...
func (c *cpolrdb) Delete(ctx context.Context, name string) error {
	...
	if err != nil {
		klog.ErrorS(err, "DB: Failed to delete cpolr from primary database")

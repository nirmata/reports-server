...
func TestEphemeralReportStore(t *testing.T) {
	// Sample ephemeral report for testing
	sampleReport := &reportsv1.EphemeralReport{
		Name:      "test-report",
	...
	tests := []struct {
	...
	}{
		{
			...
			setupMocks: func(mock *sqlmock.Sqlmock) {
				...
				(*mock).ExpectQuery("SELECT report FROM ephemeralreports WHERE clusterId = \\$1").
					WithArgs("test-cluster").
					WillReturnRows(rows)
			},
			namespace:      "",
			expectedError:  nil,
			expectedResult: []*reportsv1.EphemeralReport{sampleReport},
		},
		{
			...
			setupMocks: func(mock *sqlmock.Sqlmock) {
				reportJSON, _ := json.Marshal(sampleReport)
				rows := sqlmock.NewRows([]string{"report"}).AddRow(string(reportJSON))
				(*mock).ExpectQuery("SELECT report FROM ephemeralreports WHERE namespace = \\$1 AND clusterId = \\$2").
		...
		},
		{
			...
			namespace:      "",
			expectedError:  nil,
			expectedResult: []*reportsv1.EphemeralReport{},
		},
		{
			...
			setupMocks: func(mock *sqlmock.Sqlmock) {
				reportJSON, _ := json.Marshal(sampleReport)
				rows := sqlmock.NewRows([]string{"report"}).AddRow(string(reportJSON))
				(*mock).ExpectQuery("SELECT report FROM ephemeralreports WHERE \\(namespace = \\$1\\) AND \\(name = \\$2\\) AND \\(clusterId = \\$3\\)").
		...
		},
		{
			...
			expectedError: errors.New("no such ephemeral report"),
		},
		{
			...
			setupMocks: func(mock *sqlmock.Sqlmock) {
				reportJSON, _ := json.Marshal(sampleReport)
				(*mock).ExpectExec("INSERT INTO ephemeralreports \\(name, namespace, report, clusterId\\) VALUES \\(\\$1, \\$2, \\$3, \\$4\\)").
		...
		},
		{
			...
			setupMocks: func(mock *sqlmock.Sqlmock) {
				reportJSON, _ := json.Marshal(sampleReport)
				(*mock).ExpectExec("UPDATE ephemeralreports SET report = \\$1 WHERE \\(namespace = \\$2\\) AND \\(name = \\$3\\) AND \\(clusterId = \\$4\\)").
		...
		},
		{
			...
			operation: "delete",
			setupMocks: func(mock *sqlmock.Sqlmock) {
				(*mock).ExpectExec("DELETE FROM ephemeralreports WHERE \\(namespace = \\$1\\) AND \\(name = \\$2\\) AND \\(clusterId = \\$3\\)").
					WithArgs("test-namespace", "test-report", "test-cluster").
					WillReturnResult(sqlmock.NewResult(1, 1))
			},
			report:        sampleReport,
